...
One test is deliberately named `[known bug]` — `src/app/(main)/signup/user/actions.js`'s `signup_mentor()` calls `auth.updateUser({ data: { role: "user" } })`, stamping `user_metadata.role` as `"user"` even though this signs a *mentor* up. It's left unfixed (out of scope so far) but documented so it isn't lost; update/remove that test if it's fixed. (Three related bugs — an `app_metadata.role`-based admin check in `api/mentor/route.js`, a no-op password-confirmation check in `signup/mentor/actions.js`, and unsanitized path segments in `api/r2_upload/route.js` — were already found and fixed, each with a regression test.)
