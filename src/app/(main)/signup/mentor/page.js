...
import { login, signupMentor, handleVerifyOtp } from "./actions";
...
export default function LoginPage() {
  ...
  const handleSignup = async (prevState, formData) => {
    const result = await signupMentor(prevState, formData);
    if (result?.error) {
  ...
  return (
    ...
    <div className="bg-linear-to-br from-gray-50 to-gray-100 min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full">
        ...
        <h2 className="text-2xl font-bold text-center mb-6">メンター登録</h2>
        {/* Login Card */}
