...
describe("uploadAvatar server action", () => {
  ...
  it("rejects unauthenticated requests", async () => {
    ...
    const res = await uploadAvatar({ name: "avatar.png", type: "image/png" });

    expect(res.status).toBe(401);
  });

  it("uploads to R2 under the user's role/id namespace and saves the returned key", async () => {
    const usersChain = createChain({ error: null });
    createClient.mockResolvedValue(
      createSupabaseMock({
        auth: {
  ...
  it("saves to the mentors table and redirect path for a mentor", async () => {
    ...
    createClient.mockResolvedValue(
      createSupabaseMock({
        auth: {
          getUser: vi.fn(async () => ({
            data: { user: { id: "mentor-1", user_metadata: { role: "mentor" } } },
          })),
        },
